...
export default class Generator {
  ...
  constructor(config: IConfig) {
    ...
    Jimp.loadFont(config.fonts.large).then(font => {
      this.fonts.large = font;
    });
    Jimp.loadFont(config.fonts.medium).then(font => {
      this.fonts.medium = font;
    });
    Jimp.loadFont(config.fonts.small).then(font => {
      this.fonts.small = font;
    });
    Jimp.loadFont(config.fonts.xsmall).then(font => {
  ...
  public async makeReportCard(data: IReportCardData) {
    ...
    let scaledPortrait = portrait.cover(
      this.config.loc.portraitScale[0],
      this.config.loc.portraitScale[1]
    );
    let reportCard = template.composite(
      scaledPortrait,
      this.config.loc.portrait[0],
      this.config.loc.portrait[1],
      //@ts-ignore
  ...
  private printText(templateObj: Jimp, data: IReportCardData) {
    ...
    return (
      templateObj
        .print(
          this.fonts.medium,
          this.config.loc.name[0],
          this.config.loc.name[1],
          data.name,
          this.config.nameMaxWidth
        )
        .print(
          this.fonts.xsmall,
          this.config.loc.updated[0] - d,
          this.config.loc.updated[1],
          updatedStr
        )
        // @ts-ignore
        .print(
          this.fonts.medium,
          this.config.loc.district[0],
          this.config.loc.district[1],
          districtStr!
        )
        .print(
          this.fonts.large,
          this.config.loc.voting[0],
          this.config.loc.voting[1],
          data.grades.Voting
        )
        .print(
          this.fonts.large,
          this.config.loc.rhetoric[0],
          this.config.loc.rhetoric[1],
          data.grades.Rhetoric
        )
        .print(
          this.fonts.large,
          this.config.loc.donation[0],
          this.config.loc.donation[1],
          data.grades.Donation
        )
    );
  }
}
